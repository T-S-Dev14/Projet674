...
void player_init(Player *p, int screenWidth, int screenHeight) {
    ...
    p->flame_frame = 0;
    p->flame_timer = 0;  // Timer pour l'animation
}
...
void player_update(Player *p, const Uint8 *keystates) {
    ...
    // ---------- Animation de la flamme (CORRIGÉ) ----------
    p->flame_timer++;
    if (p->flame_timer >= 10) {      // Changement toutes les 10 frames (~6 fois/sec)
        p->flame_frame = (p->flame_frame + 1) % 3;  // 0 → 1 → 2 → 0
        p->flame_timer = 0;           
    }
} 
...
void player_render(Player *p, SDL_Renderer *renderer) {
    if (g_sprite_manager) {
        ...
        switch (p->flame_frame) {
            ...
            default: flame_sprite = FLAMME_1; break;  // Sécurité
        }

        // Flamme en dessous du vaisseau (léger overlap)
...
void player_handle_shooting(Player *p, const Uint8 *keys, Bullet bullets[]) {
    ...
    if (space && !prev_space) {
        // Position corrigée pour centrer la balle
        bullet_shoot(bullets, MAX_BULLETS, p->x + 4, p->y);
    }
